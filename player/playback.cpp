...
Playback::Playback(const DataReader &file,
    ...
    Version = std::make_unique<trc::Version>(version, pic, spr, dat);
...
void Playback::ProcessPackets() {
    ...
    int64_t playback_tick = GetPlaybackTick();

    while (Needle != Recording->Frames.cend() &&
           Needle->Timestamp.count() <= playback_tick) {
        for (auto &event : Needle->Events) {
            event->Update(*Gamestate);
        }
...
void Playback::Skip(int64_t by) {
    ...
    if (by < 0) {
        ...
        BaseTick = std::min(BaseTick + by, Recording->Runtime.count());
    }
}
